					...
					for (int i = 0; i < incList.getLength(); i++) {
						...
						if (plannedNode.getNodeType() == Node.ELEMENT_NODE
							...
							for (int j = 0; j < plannedAttr.getLength(); j++) {
								...
								if (attr1.getName().equals("label")) {
									plannedIncrement.setAnnoLabel(attr1.getValue());
								}
								if (attr1.getName().equals("position")) {
									plannedIncrement.setAnnoPosition(Double
											.parseDouble(attr1.getValue()));
								}
							}
...
//						chartVariable.setGlobal(chartAttr.getValue());
					...
					if (chartAttr.getName().equals("pr")) {
							chartVariable.setpr(chartAttr.getValue());
					}
					if (chartAttr.getName().equals("step")) {
						if(chartAttr.getValue().equals("1"))
							chartVariable.setStep(1);
						else
							chartVariable.setStep(0);
//						chartVariable.setGlobal(chartAttr.getValue());
					}
					if (chartAttr.getName().equals("Fstep")) {
						if(chartAttr.getValue().equals("1"))
							chartVariable.setFStep(1);
						else
							chartVariable.setFStep(0);
//						chartVariable.setGlobal(chartAttr.getValue());
					}
					// ChartLevelNodes
