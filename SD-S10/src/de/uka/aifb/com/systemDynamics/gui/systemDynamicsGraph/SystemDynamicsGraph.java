...
public class SystemDynamicsGraph extends JGraph implements GraphModelListener {
	...
	public SharedNodeGraphCell createSharedNodeGraphCell(double x, double y, int shareSubModel, String SharedPointer, String SharedName, double nodeVal, AbstractNode source) {
		...
		if(source instanceof AuxiliaryNode){
			((AuxiliaryNode) source).addSharedNode(sharedNode);
			sharedNode.setSource(source);
		}
		else if(source instanceof LevelNode){
	...
	/**
	 * Sets a new min value for a level node.
	 * 
	 * @param vertex vertex with new min value
	 * @param newMinValue new min value
	 */
	private void setMinValue(LevelNodeGraphCell vertex, double newMinValue) {
		if (vertex == null) {
			throw new IllegalArgumentException("'vertex' must not be null.");
		}
		//apply changed value to the attribute list
	...
	/**
	 * Sets a new max value for a level node.
	 * 
	 * @param vertex vertex with new max value
	 * @param newMaxValue new max value
	 */
	private void setMaxValue(LevelNodeGraphCell vertex, double newMaxValue) {
		if (vertex == null) {
			throw new IllegalArgumentException("'vertex' must not be null.");
		}
		//apply changed value to the attribute list
	...
	/**
	 * Sets a new characteristic behaviour for a level node.
	 * 
	 * @param vertex vertex with new characteristic behaviour 
	 * @param newCurve new characteristic behaviour
	 */
	private void setCurveValue(LevelNodeGraphCell vertex, double newCurve) {
		if (vertex == null) {
			throw new IllegalArgumentException("'vertex' must not be null.");
		}
		//apply changed value to the attribute list
			...
			if (cell != null) {
				if (cell instanceof DefaultGraphCell && !(cell instanceof DefaultEdge)) {
					...
					if (cell instanceof LevelNodeGraphCell) {
						...
						changeStartValueMenuItem.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								...
								{
									minParameter = LevelNode.MIN_START_VALUE;
									maxParameter = LevelNode.MAX_START_VALUE;
								}	
								else
								{
									minParameter = minValue;
									maxParameter = maxValue;
								}
					...
					if (cell instanceof LevelNodeGraphCell) {
						...
						JMenuItem changeMinValueMenuItem =
								new JMenuItem("Change Minimum Value");
						changeMinValueMenuItem.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								double initialMinValue = ((LevelNode)graphNode2modelNode.get(cell)).getMinValue();
								Double newMinValue =
										MinMaxDialog.showMinMaxDialog(
												start, frame,
												"Change Minimum Value",
												"Minimum Value",
												initialMinValue);
								if (newMinValue != null) {
									setMinValue((LevelNodeGraphCell)cell, newMinValue);
								}
							}
						});
						menu.add(changeMinValueMenuItem);
					}

					if (cell instanceof LevelNodeGraphCell) {
						// change maximum value
						JMenuItem changeMaxValueMenuItem =
								new JMenuItem("Change Maximum Value");
						changeMaxValueMenuItem.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								double initialMaxValue = ((LevelNode)graphNode2modelNode.get(cell)).getMaxValue();
								Double newMaxValue =
										MinMaxDialog.showMinMaxDialog(
												start, frame,
												"Change Maximum Value",
												...
												initialMaxValue);
								if (newMaxValue != null) {
									setMaxValue((LevelNodeGraphCell)cell, newMaxValue);
								}
							}
						});
						menu.add(changeMaxValueMenuItem);
					}

					if (cell instanceof LevelNodeGraphCell) {
						// change curve
						JMenuItem changeCurveMenuItem =
								new JMenuItem("Change Curve Type");
						changeCurveMenuItem.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								double initialCurveValue = ((LevelNode)graphNode2modelNode.get(cell)).getCurveValue();
								Double newCurveValue =
										CurveDialog.showCurveDialog(
												start, frame,
												"Change Curve Type",
												"Non-Decreasing",
												"Non-Increasing",
												"NULL",
												initialCurveValue);
								if (newCurveValue != null) {
									setCurveValue((LevelNodeGraphCell)cell, newCurveValue);
								}
							}
						});
						menu.add(changeCurveMenuItem);
					}
...
//					if (cell instanceof AuxiliaryNodeGraphCell || cell instanceof RateNodeGraphCell || cell instanceof SharedNodeGraphCell) {
	...
	public String getToolTipText(MouseEvent event) {
		...
		if (cell instanceof LevelNodeGraphCell) {
			...
			+ messages.getString("SystemDynamicsGraph.StartValue") + ": " + formatter.format(levelNode.getStartValue()) + "<br>"
			+ "Minimum Value" + ": " + formatter.format(levelNode.getMinValue()) + "<br>"
			+ "Maximum Value" + ": " + formatter.format(levelNode.getMaxValue()) + "</html>";
		}
